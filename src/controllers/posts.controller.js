...
console.log(title, content, type, adminId );
    ...
    try {
        ...
        const [adminRows] = await connection.execute('SELECT role FROM admins WHERE id = ?', [adminId]);
        ...
        if (!adminRole.includes(type)) {
            return res.status(403).json({ message: `You do not have permission to create a post of type '${type}'.` });
        }

        // Insert the new post into the database
        const [result] = await connection.execute(
            'INSERT INTO Posts (title, content, type, adminId, imageUrl) VALUES (?, ?, ?, ?, ?)',
            [title, content, type, adminId, imageUrl]
        );
...
exports.getPostById = async (req, res) => {
    ...
    try {
        ...
        await connection.execute('UPDATE Posts SET viewCount = viewCount + 1 WHERE id = ?', [id]);
...
exports.updatePost = async (req, res) => {
    ...
    try {
        ...
        // Check if the admin's role contains the type
        if (!adminRole.includes(type)) {
            return res.status(403).json({ message: `You do not have permission to update a post of type '${type}'.` });
        }

        // Fetch the current post to get the old image URL (if any)
        const [currentPost] = await connection.execute('SELECT imageUrl FROM Posts WHERE id = ?', [id]);
